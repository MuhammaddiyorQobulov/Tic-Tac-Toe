...
function clickedBox(element) {
  if (players.classList.contains("player")) {
    playerSign = "O";
    element.innerHTML = `<i class="${playerOIcon}" style="color : blue;"></i>`;
    players.classList.remove("active");
    element.setAttribute("id", playerSign);
  } else {
    element.innerHTML = `<i class="${playerXIcon}" style="color : red ;"></i>`;
    element.setAttribute("id", playerSign);
  ...
  if (runBot) {
    ...
    if (array.length > 0) {
      if (players.classList.contains("player")) {
        ...
        allBox[
          randomBox
        ].innerHTML = `<i class="${playerXIcon}" style="color : red ;"></i>`;
        allBox[randomBox].setAttribute("id", playerSign);
        players.classList.add("active");
      } else {
        allBox[
          randomBox
        ].innerHTML = `<i class="${playerOIcon}" style="color : blue;"></i>`;
        players.classList.remove("active");
        allBox[randomBox].setAttribute("id", playerSign);
      }
      selectWinner();
